...
const WithCoordinatesHighTextu = () => {
  ...
  let model = useRef<THREE.Object3D | null>(null);
  let floorTexture = useRef<THREE.Texture | null>(null);
  ...
  useEffect(() => {
    ...
    loader.load("/bluemap/Map2.glb", (gltf) => {
      // loader.load("/TestFBX/Mesh_all.glb", (gltf) => {

      model.current = gltf.scene;
      model.current.scale.set(6, 6, 6);
      ...
      floorTexture.current = textureLoader.load("/BridgeToHeaven.jpg");
      floorTexture.current.wrapS = THREE.RepeatWrapping;
      floorTexture.current.wrapT = THREE.RepeatWrapping;

      // applyRandomColors(model);

      // traverse though meshes
      updateMaterial("envMap", neutralEnvironment);
  ...
  const updateMaterial = (mapType, texture) => {
  ...
  const onMouseClick = (event) => {
    ...
    const baseTexture = textureLoader.load(
      "/final/B_D_A_baseColor_reserved.png"
    );

    updateMaterial("baseMap", baseTexture);
...
export default WithCoordinatesHighTextu;
